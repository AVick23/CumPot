...
async def ask_photo_flag(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id=None) -> int:
    flow = context.user_data.get("add_flow") or {}
    location, category = flow.get("location"), flow.get("category")
    breadcrumb = get_breadcrumb(location, category)
...
async def ask_notification_flag(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id=None) -> int:
    flow = context.user_data.get("add_flow") or {}
    location, category = flow.get("location"), flow.get("category")
    breadcrumb = get_breadcrumb(location, category)
...
async def finish_add(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id=None) -> int:
    flow = context.user_data.get("add_flow") or {}
...
# ---------- РЕДАКТИРОВАНИЕ ----------
async def prompt_edit_text(update: Update, context: ContextTypes.DEFAULT_TYPE, item_id, message_id=None) -> int:
    item = get_item(item_id)
    if not item:
        return await show_items_list(update, context, message_id, notice="⚠️ Пункт не найден.")
    context.user_data["last_item_id"] = item_id
...
async def cancel_edit_text(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id=None) -> int:
...
async def show_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE, item_id, message_id=None) -> int:
    item = get_item(item_id)
    if not item:
        return await show_items_list(update, context, message_id, notice="⚠️ Пункт не найден.")
...
async def confirm_delete(update: Update, context: ContextTypes.DEFAULT_TYPE, item_id, message_id=None) -> int:
    item = get_item(item_id)
    if not item:
...
async def edit_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if prefix == CB_ADD_DAY_PREFIX:
        # Редактирование дней для weekly (используем в карточке)
        try:
            item_id = int(value)
            return await edit_days(update, context, item_id, message_id)
        except (TypeError, ValueError):
            return await show_items_list(update, context, message_id)
...
# ---------- ВСПОМОГАТЕЛЬНЫЕ ----------
async def back_from_add_text(update: Update, context: ContextTypes.DEFAULT_TYPE, message_id=None) -> int:
    flow = context.user_data.get("add_flow") or {}
    context.user_data.pop("await_text", None)
