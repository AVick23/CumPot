...
def save_user(tg_id: int, username: str | None = None, first_name: str | None = None, last_name: str | None = None):
    """
    Создаёт пользователя или обновляет метаданные из Telegram.
    НЕ затирает full_name и position — они управляются только через update_user_profile.
    """
    ...
    with get_connection() as conn:
        ...
        if existing:
            ...
            # Обновляем first/last только если переданы непустые значения
